...
Set the loads on the upstream leaf nodes equal to the sum of all the loads in the
downstream nodes. It is important that the order of `nets` is from leaf branches to trunk branches
